...
3. **Аварийные сообщения скрипта идут в stderr.** Реализация запускает его
   через `cmd.Output()`: stdout попадает в результат, а в текст ошибки —
   только stderr. Обычные шаги скрипт по-прежнему пишет в stdout, и всё
   дублируется в syslog через `logger` (диагностика по ssh — `logread`).
