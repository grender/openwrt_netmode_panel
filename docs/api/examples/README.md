...
| `nikki-rulesets-rules.json` | `GET /api/nikki/rulesets` | набор `youtube` плюс два своих правила владельца в порядке файла: `worldsimseries.com` с поддоменами в туннель и подсеть `100.64.0.0/10` напрямую. Отпечаток посчитан от выбора с правилами — у выбора без них он совпадает с прежним (`nikki-rulesets-only.json` его не меняла) |
