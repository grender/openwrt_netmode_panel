...
| `status-*.json` → `ap.clients` | во всех примерах `null` | демон его ТЕПЕРЬ ЗАПОЛНЯЕТ — из `ubus call iwinfo assoclist` домашней точки. В примерах он остаётся `null` потому, что тела этой команды в `raw/` нет: в разведке она прошла через `grep mac` (`raw/91:83`), и фикстуры фейка для неё не существует. `null` здесь значит «спросить не смогли», а не «клиентов нет» — ноль тоже ответ, и он отличается. **NEEDS RECON:** `ubus call iwinfo assoclist '{"device":"<ifname AP>"}'` целиком, без grep |
| `watch-hosts.json` → `kind` | вид связи у каждого устройства | форма ответа `assoclist` **не снята целиком**: в `raw/91` команда прошла через `grep mac`. Обёртка `{"results": […]}` выведена из отступов той выдачи и из соседнего `raw/23`. Разбор поэтому терпимый: не совпало — все устройства становятся `unknown`, а не «кабель». **NEEDS RECON:** та же команда, что строкой выше |
...
| `watch-hosts.json` | `GET /api/watch/hosts` | семь аренд из `raw/91` плюс говорящий адрес без аренды (`192.168.9.238` — аренда пережила устройство, `192.168.9.99` — статика или виртуалка за relayd). Больше половины устройств **без имени**: dnsmasq пишет `*`, и наружу это уходит пустой строкой, а не звёздочкой |
