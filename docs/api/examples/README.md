...
| `status-empty.json` | `GET /api/status` | свежая установка — 200, нормальное состояние. Заодно единственный пример, где все три `links` — `null`: «хост из `Host` вывести не удалось» |
