...
| `status-*.json` → `ap.clients` | во всех примерах `null` | демон его заполняет из `ubus call iwinfo assoclist`; форма снята целиком (`raw/92`, 4 клиента). Примеры остаются `null`, потому что собраны раньше этой разведки и описывают другие моменты роутера, а подставить в них число из другого снимка значило бы склеить два роутера в один. `null` здесь значит «спросить не смогли», а не «клиентов нет» |
| `watch-hosts.json` → `kind` | вид связи у каждого устройства | форма `assoclist` снята (`raw/92`), но аренды и ассоциации в примере из разных снимков. Правило вида связи: в `assoclist` — `wireless`; в аренде, не в `assoclist` и **говорит** — `wired`; молчит — `unknown`, потому что аренда живёт часами дольше ассоциации |
| `watch.json` → числа и времена | байты, скорости, счётчики соединений | имена адресатов, цепочки и текст ошибки дословны из `raw/91`; величины трафика правдоподобны по порядку, но снимались не с них — полный ответ `/connections` весит 107 КБ и в `raw/` не помещается (`TestLimitCoversRealRouterOutput`) |
...
| `nikki-rulesets-{profile,only,foreign,rules}.json` | времена скачивания и числа правил | форма подтверждена исходниками mihomo (`docs/recon/evidence.json` → `GET /providers/rules`), но живого ответа с роутера не снимали. Отпечатки НЕ синтетические: посчитаны `rulesets.Fingerprint` от того же выбора |
...
| `nikki-rulesets-only.json` | `GET /api/nikki/rulesets` | остальной трафик напрямую (`policy: direct`), три набора в туннель (`action: tunnel`) и все три исхода сверки с движком сразу: `youtube` скачан (`loaded: true`, 1284 правила), `telegram` — набор с подсетями (`ip: true`), скачан целиком, `openai` выбран, но не скачан (`loaded: false`, `rules: 0`, `updated_at: null`). Последняя строка — самая частая жалоба владельца («включил, не работает»), и ради неё поля `loaded` и `rules` в ответе и стоят |
| `nikki-rulesets-rules.json` | `GET /api/nikki/rulesets` | набор `youtube` плюс два своих правила владельца в порядке файла: `worldsimseries.com` с поддоменами в туннель и подсеть `100.64.0.0/10` напрямую, у обоих комментарий. Отпечаток посчитан от выбора с правилами **и комментариями** (комментарий — часть выбора); у выбора без правил он совпадает с прежним (`nikki-rulesets-only.json` его не меняла) |
...
| `watch-hosts.json` | `GET /api/watch/hosts` | семь аренд из `raw/91` плюс говорящий адрес без аренды (`192.168.9.99` — статика или виртуалка за relayd). У `neural` и `192.168.9.238` вид связи `unknown`, а не `wired`: оба в аренде, оба не в `assoclist` и оба молчат — а это может быть и ПК с выдернутым кабелем, и телефон, ушедший из дома. Больше половины устройств **без имени**: dnsmasq пишет `*`, и наружу это уходит пустой строкой, а не звёздочкой |
| `watch.json` | `GET /api/watch` | сессия 12 минут за `192.168.9.219`. Все три вердикта сразу: `194.221.250.50` не дозвонился напрямую (лечится правилом «в туннель»), `discord.com` не дозвонился **через туннель** — там правило не поможет, и `error_chain` называет узел; `cdn.discordapp.com` молчит и помечен новым; остальные `ok`. Страна стоит ровно у одной строки из шести — столько её и бывает |
