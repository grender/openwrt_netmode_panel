...
`links.b4` в остальных шести файлах — `http://192.168.9.1:7000/`, а
`links.nikki` — `http://192.168.9.1:9090/ui/`, то есть
ответ на запрос с `Host: 192.168.9.1:8088`. В `links.nikki` **нет и не будет
секрета**: там адрес без параметров, а полный (с `?secret=`) отдаёт
`GET /api/nikki/panel` по клику — фикстуры под него нет намеренно, тело
с секретом не место в примерах. Значение **зависит от запроса**, а
