...
Статус: **реализована, см. [ADR-0039](../../adr/0039-geosite-rulesets-mixin-file.md)**
(согласована владельцем 2026-09-07; заменяла черновик с UCI-хранилищем и снимком
каталога в бинаре). Пошаговый план реализации —
...
- **Ноль наборов при `only`/`except`: ключ `rule-providers:` не пишется вовсе.**
  Пустая рубрика — это `rule-providers: null`, и слияние `yq` (`. * $item`)
  затёрло бы этим значением провайдеров профиля владельца. Файл в этом случае —
  шапка плюс `nikki-rules` из двух хвостовых правил.
