...
func run() error {
	...
	//
	// Расписание запускается ВСЕГДА, в том числе без адреса подписки.
	//
	// Раньше без адреса оно не запускалось вовсе — и адрес, заданный потом
	// из панели (PUT /api/subscription, ADR-0034), так и оставался без
	// планового обновления до перезапуска демона: кнопка работала, а
	// двенадцатичасовое обновление молчало, и заметить это было нечем.
	// Историю неудач пустой адрес при этом не копит: пока адреса нет,
	// обёртка расписания джоб не заводит (schedGate), а RunOnce на
	// ErrNotConfigured журнал не пишет.
	if cfg.SubscriptionURL == "" {
		log.Printf("адрес подписки не задан: расписание ждёт, пока его зададут в панели; " +
