...
if [ "$INSTALL" = yes ]; then
	...
	# скачивает и раскладывает сам демон (SPEC §9). Второго пути обновления
...
2. Убрать строку happ2clash из crontab — чужой crontab демон не правит:
   ...
   скачивает демон (SPEC §9). Оставленная строка cron будет звать
   несуществующий файл и слать письма root.
EOF
fi
