...
export const DICT = {
	ru: {
		...
		'bridge.fail.executor_missing.text': 'Настройки записаны, но применить их нечем: скрипта нет. Установите пакет заново (deploy.sh --install) и повторите.',
		'bridge.fail.install_failed.title': 'Не удалось установить relayd',
		'bridge.fail.install_failed.text': 'apk не смог поставить пакет — обычно нет интернета через внешнюю сеть или места на флеш-памяти. Конфигурация не тронута.',
		'bridge.fail.no_iface.title': 'Интерфейс проброса не поднялся',
		'bridge.fail.no_iface.text': 'Настройки применены, но нога роутера во внешней сети не появилась с нужным адресом. Проверьте, свободен ли адрес, и смотрите logread.',
		'bridge.fail.relay_down.title': 'Служба relayd не в нужном состоянии',
		'bridge.fail.relay_down.text': 'Интерфейс поднят, а мост не работает: relayd не запустился (или не остановился при выключении). ПК не виден внешней сети — смотрите logread.',
		'bridge.fail.unverifiable.title': 'Исход неизвестен',
		'bridge.fail.unverifiable.text': 'Настройки применены, но проверить результат не удалось: ubus не ответил. Успех и провал равновозможны — обновите панель или смотрите ssh.',
		'bridge.fail.stale_draft.title': 'В конфигурации застрял черновик',
		'bridge.fail.stale_draft.text': 'Операция не состоялась, и отменить недописанные правки роутер не смог. Повтор не поможет — панель будет ссылаться на незакоммиченные правки, а в LuCI пусто: черновик наш. Зайдите по ssh: uci revert network; uci revert firewall',
		...
		'bridge.fail.unknown.text': 'Роутер сообщил о неудаче, но причину панель не распознала. Разные версии панели и демона — обновите панель; иначе причина придёт следующим опросом.',
		...
		'bridge.down': 'Состояние проброса не прочитано',
		'bridge.unknown': 'не прочитано',
		'bridge.state.on': 'включён',
		'bridge.state.off': 'выключен',
		'bridge.on.text': 'ПК {ip} виден клиентам внешней сети через VLAN 2 на LAN-порту.',
		'bridge.off.text': 'ПК за LAN-портом живёт в сети роутера и снаружи не виден. Включите проброс, чтобы он появился во внешней сети под своим адресом.',
		'bridge.diag.state': 'Проброс',
		'bridge.diag.relayd': 'Служба relayd',
		'bridge.diag.port': 'LAN-порт',
		'bridge.diag.flaps': 'Перескоков линка',
		'bridge.diag.uplink': 'Внешняя сеть',
		'bridge.diag.leg': 'Адрес роутера в ней',
		'bridge.diag.pc': 'Отклик ПК',
		'bridge.diag.gw': 'Отклик шлюза',
		'bridge.relayd.absent': 'не установлена',
		'bridge.relayd.running': 'работает',
		'bridge.relayd.stopped': 'не запущена',
		'bridge.port.up': '{name}: линк есть, {speed} Мбит/с',
		'bridge.port.down': '{name}: линка нет — кабель не подключён',
		// Счётчик перескоков — не украшение: физический флап линка молча ломал
		// схему, и заметить его удалось только по этому числу.
		'bridge.flaps.many': '{n} — линк нестабилен, проверьте кабель',
		'bridge.uplink.up': '{addr}/{mask}, шлюз {gw}',
		'bridge.uplink.down': 'не подключена',
		'bridge.probe.ok': 'отвечает',
		'bridge.probe.silent': 'молчит',
		'bridge.enable': 'Включить проброс',
		'bridge.enabling': 'Включаю…',
		'bridge.disable': 'Выключить проброс',
		'bridge.disabling': 'Выключаю…',
		'bridge.probe': 'Проверить связь',
		'bridge.probing': 'Проверяю…',
		'bridge.access.label': 'Доступ к внешней сети из Wi-Fi роутера',
		'bridge.access.on': 'Открыт',
		'bridge.access.off': 'Закрыт',
		'bridge.access.hint': 'Устройства вашей Wi-Fi смогут обращаться к внешней сети. Обратное направление это не открывает.',
		'bridge.confirm.disable': 'Выключить проброс? VLAN, интерфейсы и правила firewall будут сняты полностью, ПК перестанет быть виден во внешней сети. Пакет relayd останется.',
		'bridge.confirm.install': 'Для проброса нужен пакет relayd, его нет на роутере. Установить его сейчас (apk add relayd)? Без установки ничего сделано не будет.',
		'bridge.sheet.title': 'Включить проброс LAN в uplink',
		'bridge.sheet.intro': 'Внешняя сеть сейчас {net}. Оба адреса ниже обязаны быть из неё.',
		// Предупреждение о разрыве — в форме, до нажатия: применение
		// пересобирает бридж, и панель у ПК за этим портом теряет связь.
		'bridge.sheet.warn': 'Применение на секунды пересоберёт сеть роутера: связь с панелью может прерваться. Дождитесь и обновите страницу.',
		'bridge.field.leg': 'Адрес роутера во внешней сети',
		'bridge.field.leg.hint': 'Свободный адрес: панель проверит и откажется его брать, если занят.',
		'bridge.field.pc': 'Адрес ПК',
		'bridge.field.pc.hint': 'Тот, что настроен на самом ПК: панель его не назначает, а запоминает и проверяет отклик.',
		'bridge.field.access': 'Открыть доступ к внешней сети из Wi-Fi роутера',
		'bridge.field.access.hint': 'Можно включить и позже, кнопкой на этой вкладке.',
		'bridge.err.leg': 'Адрес роутера не похож на IPv4',
		'bridge.err.pc': 'Адрес ПК не похож на IPv4',
		'bridge.err.uplink': 'Внешняя сеть не подключена — проброс мостит LAN именно в неё',
		'bridge.err.subnet': 'Оба адреса обязаны быть из подсети внешней сети ({net})',
		'bridge.err.conflict': 'Адреса роутера, ПК и шлюза обязаны быть разными',
	...
	en: {
		...
		'bridge.fail.executor_missing.text': 'Settings are written, but there is nothing to apply them with. Reinstall the package (deploy.sh --install) and retry.',
		'bridge.fail.install_failed.title': 'Could not install relayd',
		'bridge.fail.install_failed.text': 'apk failed to install the package — usually no internet over the uplink, or no space in flash. The configuration is untouched.',
		'bridge.fail.no_iface.title': 'Bridge interface did not come up',
		'bridge.fail.no_iface.text': 'Settings were applied, but the router leg never appeared in the uplink with the expected address. Check the address is free and look at logread.',
		'bridge.fail.relay_down.title': 'The relayd service is in the wrong state',
		'bridge.fail.relay_down.text': 'The interface is up but the bridge is not working: relayd did not start (or did not stop when disabling). The PC is not visible to the uplink — check logread.',
		'bridge.fail.unverifiable.title': 'Outcome unknown',
		'bridge.fail.unverifiable.text': 'Settings were applied, but the result could not be verified: ubus did not answer. Success and failure are equally possible — reload or check over ssh.',
		'bridge.fail.stale_draft.title': 'A draft is stuck in the configuration',
		'bridge.fail.stale_draft.text': 'The operation did not go through and the router could not revert the half-written changes. Retrying will not help — the panel will cite uncommitted changes while LuCI shows none: the draft is ours. Over ssh run: uci revert network; uci revert firewall',
		'bridge.fail.unknown.title': 'The operation failed, reason unknown',
		'bridge.fail.unknown.text': 'The router reported a failure but the panel did not recognise the reason. Different panel and daemon versions — update the panel; otherwise it arrives with the next poll.',
		'tabs.wifi': 'Wi-Fi',
		'tabs.bypass': 'Bypass',
		'tabs.bridge': 'Bridge',
		'job.hint': 'The panel runs one operation at a time',
		'bridge.title': 'LAN-to-uplink bridge',
		'bridge.down': 'Bridge state could not be read',
		'bridge.unknown': 'not read',
		'bridge.state.on': 'on',
		'bridge.state.off': 'off',
		'bridge.on.text': 'PC {ip} is visible to uplink clients over VLAN 2 on the LAN port.',
		'bridge.off.text': 'The PC behind the LAN port lives in the router network and is invisible outside. Turn the bridge on to give it its own uplink address.',
		'bridge.diag.state': 'Bridge',
		'bridge.diag.relayd': 'relayd service',
		'bridge.diag.port': 'LAN port',
		'bridge.diag.flaps': 'Link flaps',
		'bridge.diag.uplink': 'Uplink network',
		'bridge.diag.leg': 'Router address there',
		'bridge.diag.pc': 'PC responds',
		'bridge.diag.gw': 'Gateway responds',
		'bridge.relayd.absent': 'not installed',
		'bridge.relayd.running': 'running',
		'bridge.relayd.stopped': 'not running',
		'bridge.port.up': '{name}: link up, {speed} Mbit/s',
		'bridge.port.down': '{name}: no link — cable unplugged',
		'bridge.flaps.many': '{n} — the link is unstable, check the cable',
		'bridge.uplink.up': '{addr}/{mask}, gateway {gw}',
		'bridge.uplink.down': 'not connected',
		'bridge.probe.ok': 'answers',
		'bridge.probe.silent': 'silent',
		'bridge.enable': 'Turn bridge on',
		'bridge.enabling': 'Turning on…',
		'bridge.disable': 'Turn bridge off',
		'bridge.disabling': 'Turning off…',
		'bridge.probe': 'Check connectivity',
		'bridge.probing': 'Checking…',
		'bridge.access.label': 'Access to the uplink network from the router Wi-Fi',
		'bridge.access.on': 'Open',
		'bridge.access.off': 'Closed',
		'bridge.access.hint': 'Devices on your Wi-Fi will reach the uplink network. It does not open the reverse direction.',
		'bridge.confirm.disable': 'Turn the bridge off? VLAN, interfaces and firewall rules will be removed completely and the PC will stop being visible in the uplink network. relayd stays installed.',
		'bridge.confirm.install': 'The bridge needs the relayd package, which is not on the router. Install it now (apk add relayd)? Without installing, nothing will be done.',
		'bridge.sheet.title': 'Turn on the LAN-to-uplink bridge',
		'bridge.sheet.intro': 'The uplink network is currently {net}. Both addresses below must belong to it.',
		'bridge.sheet.warn': 'Applying rebuilds the router network for seconds: the panel may lose connection. Wait and reload the page.',
		'bridge.field.leg': 'Router address in the uplink network',
		'bridge.field.leg.hint': 'A free address: the panel checks it and refuses to take it if it is in use.',
		'bridge.field.pc': 'PC address',
		'bridge.field.pc.hint': 'The one configured on the PC itself: the panel remembers it and checks whether it responds.',
		'bridge.field.access': 'Open access to the uplink network from the router Wi-Fi',
		'bridge.field.access.hint': 'Can be turned on later from this tab.',
		'bridge.err.leg': 'The router address is not a valid IPv4',
		'bridge.err.pc': 'The PC address is not a valid IPv4',
		'bridge.err.uplink': 'The uplink network is not connected — that is what the bridge bridges into',
		'bridge.err.subnet': 'Both addresses must belong to the uplink subnet ({net})',
		'bridge.err.conflict': 'Router, PC and gateway addresses must all differ',
