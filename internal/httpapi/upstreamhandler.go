...
// панель (словари в web/panel/src/i18n) и описывает docs/api/openapi.yaml. Новая причина,
...
// хвост: перевод в словарях панели и enum в openapi.
//
...
func (s *Server) switchFailed(ssid string, reason FailReason, err error) error {
	if !knownReason(reason) {
		s.logf("ВНИМАНИЕ: причина %q вне таксономии (allReasons в upstreamhandler.go) — "+
			...
			"добавьте строку в allReasons, в словари web/panel/src/i18n и в enum docs/api/openapi.yaml", reason)
	}
