...
// Read обязан отдавать КОПИЮ на обоих путях — и на попадании, и на промахе.
//
// Промах раньше возвращал тот самый указатель, который лёг в кэш, так что
// обработчик, дописавший поле в ответ, отравлял кэш на 500 мс. При опросе
// раз в секунду это давало испорченный ответ примерно раз в две секунды, а
// в тесте с одним запросом не воспроизводилось вовсе. Здесь запросов два, и
// правится ответ ПЕРВОГО — того, который пришёл с промаха.
func TestReadNeverHandsOutTheCachedStatus(t *testing.T) {
	r, _ := newReader(t)
	base := time.Unix(1700000000, 0)
	r.now = func() time.Time { return base } // оба чтения внутри TTL

	s1, _ := r.Read(context.Background()) // промах: собрано и закэшировано
	link := "http://192.168.9.1:7000/"
	s1.Links.B4 = &link // ровно то, что делает handleStatus

	s2, _ := r.Read(context.Background()) // попадание в кэш
	if s2.Links.B4 != nil {
		t.Errorf("ссылка из первого ответа протекла в кэш: links.b4 = %q", *s2.Links.B4)
	}
	if s1 == s2 {
		t.Error("Read вернул один и тот же указатель дважды — правка ответа портит кэш")
	}
}
