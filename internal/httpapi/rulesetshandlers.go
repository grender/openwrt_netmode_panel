...
func (s *Server) handleRulesetsPut(w http.ResponseWriter, r *http.Request) {
	...
	// Авто-пул — вторая, независимая секция того же файла, и этот PUT про
	// неё ничего не знает. Перенести её из прочитанного обязательно: файл
	// пишется ЦЕЛИКОМ, и без переноса применение наборов молча стёрло бы
	// выбор владельца по узлам. У испорченного файла переносить нечего —
	// там cur пустой, и это честно: его содержимое нам неизвестно.
	want.Auto = cur.Auto
