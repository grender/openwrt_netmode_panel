...
// дёргается дважды», а cron зовёт файл, которого на роутере уже нет.
// Писем об этом не будет: MTA в образе OpenWrt нет, а busybox crond собран
// без вызова sendmail, — отказ виден только штатной строкой crond в
// logread, то есть практически не виден. Строка бесполезна и вводит в
// заблуждение при следующем разборе (ADR-0031).
//
...
func (s *Scheduler) RunOnce(ctx context.Context) (happ.Summary, error) {
	...
	if errors.Is(err, subs.ErrNotConfigured) {
		// Состояние, а не поломка, и контракт сентинеля прямо говорит:
		// в журнал не писать. Сегодня сюда не доходит ни один боевой
		// путь — main не запускает расписание без адреса, обработчик
		// отбивает кнопку раньше, — но держать контракт на двух
		// предпроверках в чужих пакетах значит ждать третий путь,
		// который их не повторит, и копить fail-строки на свежей
		// установке. Импорт subs ради одного сентинеля — круг 3 в круг 3
		// (ADR-0001), цикла нет: subs о sched не знает.
		return sum, err
	}

	if len(sum.UnknownKeys) > 0 {
		// Единственный след того, что провайдер сменил формат xhttp.
		// Узлы собраны без этих ключей, и если один из них был
		// обязательным для сервера, узел «то работает, то нет» — самый
		// дорогой в поиске отказ, без единой зацепки в панели.
		s.logf("подписка принесла ключи xhttp, которых конвертер не знает: %v; узлы собраны без них",
			sum.UnknownKeys)
	}
