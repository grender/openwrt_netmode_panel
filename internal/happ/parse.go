...
func parseRecord(rec json.RawMessage, index int, xhttp bool) (Entry, string) {
	...
	if !ok {
		// Балансировщик без wl-outbound — это «Авто | Быстрый узел»:
...
//     побайтовая копия узла «Браво (РЦ-3)» вплоть до балансировщика.
//
...
func dedupeNames(entries []Entry) {
	...
	for i := range entries {
		...
		// провайдер сам прислал «Браво» и «Браво (2)». Крутим,
...
// отдельной строкой. Взяв decoy, мы завели бы дубликат под именем «РЦ-N» и
