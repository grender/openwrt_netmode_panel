...
func (tb *table) applySnapshot(snap nikki.Snapshot, srcIP string, elapsed time.Duration, now time.Time) {
	...
	for _, c := range snap.Connections {
		...
		switch {
		case !seen && !tb.primed:
			// ПЕРВЫЙ снимок сессии задаёт только точку отсчёта. Соединение,
			// открытое до начала наблюдения, несёт в upload/download всё,
			// что перенесло за свою жизнь, и засчитай мы это, «за сессию»
			// показало бы часы чужого трафика, а скорость первого тика —
			// десятки мегабайт в секунду. Найдено на живом роутере
			// 2026-09-14: 35 МБ «за сессию» через полторы минуты после старта.
			dUp, dDown = 0, 0
		case !seen || dUp < 0 || dDown < 0:
			// Появилось при наблюдении либо счётчики поехали назад (движок
			// перезапустился, id стали новыми) — берём то, что есть, за
			// прирост целиком: соединение родилось уже на наших глазах.
