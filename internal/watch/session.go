...
// LastPoll — когда сессию в последний раз продлевали.
//
// Существует ради одной проверки, и она того стоит: «статус читает сводку,
// но TTL не трогает» иначе не выражается ничем. Отличить Poll от State по
// их результату нельзя — оба отдают одно и то же состояние, — а именно на
// этой разнице стоит обещание «закрыл вкладку, через минуту погасло».
func (s *Session) LastPoll() time.Time {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.lastPoll
}
